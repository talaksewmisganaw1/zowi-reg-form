  ...
  return (
    <div className="medical-information">
        ...
        <div className="inputs">
          ...
          <label className="information-label-and-input"><p className="labels">ማጠቃለያ፡ </p><textarea className="lined-textarea" value={medicalClients[medicalResultIndex].summary} onChange={(e) => {handleChange(medicalResultIndex, "summary", e.currentTarget.value)}}></textarea></label>
          <label className="information-label-and-input"><p className="labels">የክትትል ማስታወሻ፡ </p><textarea className="lined-textarea" value={medicalClients[medicalResultIndex].followupNote} onChange={(e) => {handleChange(medicalResultIndex, "followupNote", e.currentTarget.value)}}></textarea></label>
              ...
              <div className="appointments">
                {appointments.length > 0? appointments.map((element, i) => (
                  <div className="appointment" key={i}>
                    <span className='delete-icon' onClick={() => {deleteAppointment(i)}}><FontAwesomeIcon icon={faTrash} /></span><span className="appointment-number">{i + 1}.</span><span className='appointment-date'>{element}</span>
                  </div>
